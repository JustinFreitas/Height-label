...
function onScaleChanged(tokenCT)
	...
	if nodeCT then
		TokenManager.updateNameScale(tokenCT);
		if bDisplayDefaultHealth then 
			local nPercentWounded = fGetHealthInfo(nodeCT);
			TokenManager.updateHealthBarScale(tokenCT, nPercentWounded); 
		end
...
-- NOTE: Assume registered on host (FGU) or all clients (FGC); Only called for PC (charsheet) node owner changes
...
function updateFactionHelper(tokenCT, nodeCT)
	...
	if UtilityManager.isClientFGU() then
		...
		tokenCT.setModifiable(bAllowPublicAccess);
	end
...
function updateTokenColor(token)
	-- Only update custom color if token exists and only on host (FGU only)
	if not token then
		return;
	end
...
function autoTokenScale(tokenMap)
	if UtilityManager.isClientFGU() then
		...
		local nSpace = TokenManager.getTokenSpace(tokenMap);
...
function updateHealthBarScale(tokenCT, nPercentWounded)
	...
	if widgetHealthBar then
		if UtilityManager.isClientFGU() then
			...
			local barh = TOKEN_HEALTHBAR_FGU_HEIGHT;
			...
			widgetHealthBar.setPosition("right", TOKEN_HEALTHBAR_FGU_HOFFSET, 0);
		else
			local w, h = tokenCT.getSize();
